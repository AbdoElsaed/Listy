...
exports.login = async (req, res) => {
  try {
    ...
    if (user && bcrypt.compareSync(req.body.password, user.password)) {
      ...
      return res.header("Authorization", token).json({ user: _.pick(user, returnedValues), token });
    } else {
...
exports.addAvatar = async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.user._id, { $set: { avatar: data } }, { new: true });
    const { avatar } = user;
    return res.json({ avatar });
